    ...
    const fetchData = async () => {
      try {
        ...
        if (response.status === 'success') {
          ...
          setelevationItems(
            response.data.elevation.map(elevation => ({
              label: elevation,
              value: elevation,
            })),
          );
          setwallblowerItems(
            response.data.wall_blower.map(wallblower => ({
              label: wallblower,
              value: wallblower,
            })),
          );
          ...
          setpanellItems(
            (response.data.panel || []).map(panel => ({
              label: panel,
              value: panel,
            })),
          );
          setneckItems(
            (response.data.neck || []).map(neck => ({
              label: neck,
              value: neck,
            })),
          );
      ...
      const fetchData = async () => {
        try {
          ...
          if (response.status === 'success') {
            ...
            setCoilItems(
              response.data.coil_number.map(coil => ({
                label: coil,
                value: coil,
              })),
            );
            ...
            setTubeItems(
              response.data.tube_number.map(tube => ({
                label: tube,
                value: tube,
              })),
            );
            ...
            setwelderItems(
              response.data.welders.map(([id, name]) => ({
                label: name,
                value: id,
              })),
            );
  ...
  const handleApproveAll = async () => {
    if (selectedJobs.length > 0) {
      ...
      try {
        ...
        alert('An error occurred while submitting RT Report.');
        SetApprovemodalVisible(false); // Close the modal
      }
    } else {
      alert('No job selected. Please select one.');
    }
  };
                  ...
                  <View style={styles.rightButtons}>
                    ...
                    <TouchableOpacity
                      style={{
                        ...styles.clearButton,
                        backgroundColor:
                          filterCriteria.length > 0 ? '#FF6347' : '#4CAF50',
                      }}
